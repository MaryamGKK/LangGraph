...
async def main():
    async with stdio_client(server_params) as (read, write): # type of transportation
        async with ClientSession(read, write) as session: # defining the session
            ...
            print("Available Tools -", *[tool.name for tool in tools]) # tools names separated by space
            print('-'*60)
